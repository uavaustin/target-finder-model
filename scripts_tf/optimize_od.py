...
def get_annotations(records_dir):
  ...
  def _parse_image_function(example_proto):
    ...
    record['image/encoded'] = tf.image.decode_image(record['image/encoded'],
                          channels=3, 
                          dtype=tf.dtypes.uint8)                  
    return record
      
  dataset = raw_dataset.map(_parse_image_function, num_parallel_calls=1)
  dataset = dataset.enumerate(0)

  for i, data in dataset:
    img = {
      "file_name": str(data['image/source_id'].numpy()), 
      "height": str(data['image/height'].numpy()),
      "width": str(data['image/width'].numpy()),
      "id": str(i.numpy())
    }
    images.append(img)
    ...
    for j in range(0, data['image/object/class/label'].shape[0]):
      ...
      coco_label = {
        "id": j,
        'image_id': str(data['image/source_id'].numpy()),
...
def get_graph_func(input_saved_model_dir,
  ...
  if use_trt:
    ...
    def input_fn(input_data_dir, num_iterations):
      dataset, coco = get_dataset(
          records_dir=input_data_dir,
...
def run_inference(graph_func,
  ...
  dataset_full,  coco = get_dataset(records_dir=data_dir,
                        batch_size=batch_size,
                        input_size=input_size)
  if mode == 'validation':
    ...
    for i, dataset_batch in dataset_full:
      ...
      if i.numpy() % display_every == 0:
        print("  step %d/%d, iter_time(ms)=%.0f" %
              (i.numpy()+1, num_imgs//batch_size, iter_times[-1]*1000))
    ...
    for i, dataset_batch in dataset_full:
      if i.numpy() >= num_warmup_iterations:
        start_time = time.time()
        batch_preds = list(graph_func(dataset_batch['image/encoded']).values())[0].numpy()
...
if __name__ == '__main__':
  ...
  assert config['inputs']['detector']['width'] \
  ...
  parser.add_argument('--input_size', type=int, default=config['inputs']['detector']['width'],
  ...
  parser.add_argument('--num_warmup_iterations', type=int, default=1,
  ...
  results, predictions, coco = run_inference(graph_func,
                data_dir=args.data_dir,
                batch_size=args.batch_size,
