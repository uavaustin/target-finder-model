...
def get_dataset(data_files,
    ...
    def _parse_image_function(example_proto):
        ...
        record['image/encoded'] = tf.cast(record['image/encoded'],
    ...
    if mode == 'validation':
        ...
        for i, batch_images in dataset:
            if i >= num_warmup_iterations:
                start_time = time.time()
                batch_preds = graph_func(
                    batch_images['image/encoded'])[0].numpy()
...
if __name__ == '__main__':
    ...
    if args.use_trt and not args.output_saved_model_dir:
        raise ValueError(
            "--output_saved_model_dir must be set if use_trt=True")
