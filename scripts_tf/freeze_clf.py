...
flags = tf.app.flags
tf.flags.DEFINE_string("model_name", "inception_v3", "Model name.")
tf.flags.DEFINE_string("ckpt_dir", "", "Directory containing model ckpts.")
tf.flags.DEFINE_string("output_dir", "", "Output model dir.")
FLAGS = flags.FLAGS
...
class NetDef(object):
    ...
    def __init__(
        ...
        name,
        url=None,
        model_dir_in_archive=None,
        checkpoint_name=None,
        preprocess="inception",
        input_size=CLF_SIZE,
        slim=True,
        postprocess=tf.nn.softmax,
        model_fn=None,
...
def freeze_model(model, ckpt_dir, output_dir):
    ...
    netdef = NetDef(name=model, input_size=CLF_SIZE, num_classes=len(CLF_CLASSES))
        ...
        with tf.Session(config=tf_config) as tf_sess:
            ...
            if netdef.slim:
                ...
                network_fn = nets.nets_factory.get_network_fn(
                    netdef.name, netdef.num_classes, is_training=False
                )
            ...
            frozen_graph = tf.graph_util.convert_variables_to_constants(
                tf_sess, tf_sess.graph_def, output_node_names=["logits", "classes"]
            )
