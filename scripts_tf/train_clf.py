...
tf.app.flags.DEFINE_string(
    ...
    "exponential",
    'Specifies how the learning rate decays. One of "fixed", "exponential",'
    ' or "polynomial"',
)
...
tf.app.flags.DEFINE_integer("train_image_size", CLF_SIZE, "Train image size")
...
tf.app.flags.DEFINE_string(
    "records_name", "model_data/records/", "Prefix of classification records"
)
...
def _configure_learning_rate(num_samples_per_epoch, global_step):
    ...
    if FLAGS.learning_rate_decay_type == "exponential":
        l_rate = tf.train.exponential_decay(
            FLAGS.learning_rate,
            global_step,
            decay_steps,
...
def _get_init_fn():
    ...
    if tf.train.latest_checkpoint(FLAGS.train_dir):
        tf.logging.info(
            "Ignoring --checkpoint_path. Checkpoint already exists in %s"
            % FLAGS.train_dir
        )
        return None
...
def main(_):
    ...
    file_pattern = os.path.join(
        FLAGS.records_name, file_pattern % FLAGS.dataset_split_name
    )
